...
year_cols <- paste0('year',2001:2016)
d[, fips := as.character(GeoFIPS)]
d[nchar(fips)==4, fips := paste0('0',fips)]
for(v in as.character(2001:2016)) d[, (v) := as.numeric(get(v))]
d[, `2000` := `2001`]
...
d <- melt(d, id.vars = c('fips','Description'), measure.vars = as.character(2000:2016), variable.name = 'year')
...
## Merge on transfers broken down
per <- melt(per, id.vars = c('fips','Description'), measure.vars = as.character(2000:2016), variable.name = 'year')
per[, year := as.numeric(as.character(year))]
per[, value := as.numeric(value)]
## This one is doubled under two headings, but since we aren't using it for anything just drop.
...
## Save.
d <- d[, c('fips','year','percent_transfers','income_per_capita','total_employees')]
